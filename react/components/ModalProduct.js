...
const ModalProduct = ({ product_id, state_modal, set_state_modal }) => {
    ...
    if(!state_modal) return null;

    return (
        <div className={styles["modal-product-container"]} >
            <button onClick={()=> set_state_modal(false)} className={styles["modal-product-close-btn"]}>
            ...
            <div className={styles["modal-product-data"]}>
                <div>
                    <img src={product.image.imageUrl} alt="Product Image" />
                </div>
                <div>
                    <span>{product.productName}</span>
                    <span>{product.seller.commertialOffer.ListPrice}</span>
                    <span>{product.seller.commertialOffer.Price}</span>
                    <a href={product.link}>ver</a>
                </div>
            </div>
        </div>
    );
